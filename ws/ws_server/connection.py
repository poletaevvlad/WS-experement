...
class Connection():
	...
	def connected(self, role):
		if role in ["display", "controller"]:
			return (self.display is not None) if role == "display" else (self.controller is not None)
		return False
	...
	def connect(self, role, ws):
		...
		if self.session_code not in connections:
			connections[self.session_code] = self
